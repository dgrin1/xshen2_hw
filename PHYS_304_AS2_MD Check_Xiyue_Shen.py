...
def init_velocity(T, number_of_particles):
    ...
    R=1+np.arange(number_of_particles) # change R from random to specific values
    # so that when we run the simulation twice, it should generate the same result
    # in this way we can see if adding the y dimension change anything
...
def run_md(dt, number_of_steps, initial_temp,snaptime, x,y, number_of_atoms): #add parameters for snaptime and number_of_atoms
    ...
    vy=0 # for check, we set the initial velocity along y dimension equal to zero
    # to see if the code still work for 1 dimension
...
y=0+0*np.arange(number_of_atoms) # initiate the position on y dimension
